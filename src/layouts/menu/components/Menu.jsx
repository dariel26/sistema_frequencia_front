    ...
    return (
        <div className="d-flex flex-column bg-menu-dark-light w-100 h-100">
            ...
            <div className="d-flex flex-column w-100" id="my-menu-body"
                style={{ transition: "height 300ms", overflowX: "hidden", overflowY: "auto" }}>
                    ...
                    props.navs ? props.navs.map((nav, index) => (
                        <NavLink to={nav.path} id="my-left-nav"
                            ...
                            <div className="d-flex h-100 align-items-center"
                                ...
                                <span className="text-nowrap" style={props.isClosed ? { display: "none" } : {}}>{nav.name}</span>
                            </div>
                        </NavLink>
                    )) : undefined
                }
                ...
                style={{ height: "max-content" }}>
                {
                    props.actions ? props.actions.map((action, index) => (
                        <button key={index} onClick={action.onClick}
                            ...
                            <div className="d-flex h-100 align-items-center"
                                style={props.isClosed ? { width: "0px", transition: "width 300ms" } : { width: "calc(100% - 50px)", transition: "width 300ms" }}>
                                <span className="text-light text-nowrap" style={props.isClosed ? { display: "none" } : {}}>{action.name}</span>
                            </div>
                        </button>
                    )) : undefined
                }

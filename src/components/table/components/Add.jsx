    ...
    function emailWasAdd(user) {
      ...
      if (arrUsers.find((u) => u[indexKey] === user[indexKey]) === undefined) {
        ...
        alert.addAlert({
          ...
          message:
            "Parece que um usuário está sendo duplicado, portanto não foi adicionado.",
          time: 5000,
          key: user[uniqueKey],
        });
      }
    ...
    function readFile(event) {
      ...
      if (datas === false) {
        alert.addAlert({
          ...
          message:
            "Parece que o documento não está formatado da forma correta, ou não possui nenhum usuário",
          time: 5000,
          key: "documentinvalid",
        });
      } else {
